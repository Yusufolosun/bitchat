...
check_status() {
    ...
    stx call-read-only "$CONTRACT_ADDRESS" message-board-v4 get-total-messages \
        --mainnet 2>/dev/null || echo "  Total Messages: Unable to fetch"
    
    # Get total fees
    stx call-read-only "$CONTRACT_ADDRESS" message-board-v4 get-total-fees-collected \
        --mainnet 2>/dev/null || echo "  Total Fees: Unable to fetch"
    
    # Get pause status
    stx call-read-only "$CONTRACT_ADDRESS" message-board-v4 is-contract-paused \
